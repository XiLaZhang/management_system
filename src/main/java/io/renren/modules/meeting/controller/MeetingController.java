...
//        String roomName = roomService.queryNameById(roomId);
    ...
    @RequestMapping(value = "/applet/reserve/{userId}", method = RequestMethod.GET)
     ...
     * 判断验证码获取会议对象
     */
    @ApiOperation("小程序判断验证码获取会议对象")
    @RequestMapping(value = "/applet/code", method = RequestMethod.POST)
    public R appletEnterMeeting(@RequestParam String code){
        MeetingEntity entity = meetingService.queryMeetingByCode(code);
        return R.ok().put("meetingEntity", entity);
    }
